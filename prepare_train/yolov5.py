...
class Change2Yolov5():
    ...
    def change_image_format(self, suffix=    '.jpg'):
        """
        ...
        for extern in externs:
            files.extend(glob(self.label_path + "\\*." + extern))
        for file in files:
    ...
    def get_all_class(self, file_list):
        """
        从json文件中获取当前数据的所有类别
        ...
        for filename in file_list:
            json_path = os.path.join(self.label_path, filename + '.json')
    ...
    def split_dataset(self, test_size=0.3, isUseTest=True, useNumpyShuffle=False):
        """
        将文件分为训练集，测试集和验证集
        ...
        :return:
        """
        files = glob(self.label_path + "\\*.json")
    ...
    def create_save_file(self):
        ...
        """
        # 生成训练集
        train_image = os.path.join(self.label_path, 'train', 'images')
        if not os.path.exists(train_image):
            os.makedirs(train_image)
        train_label = os.path.join(self.label_path, 'train', 'labels')
        if not os.path.exists(train_label):
            os.makedirs(train_label)
        # 生成验证集
        val_image = os.path.join(self.label_path, 'valid', 'images')
        if not os.path.exists(val_image):
            os.makedirs(val_image)
        val_label = os.path.join(self.label_path, 'valid', 'labels')
        if not os.path.exists(val_label):
            os.makedirs(val_label)
        # 生成测试集
        test_image = os.path.join(self.label_path, 'test', 'images')
        if not os.path.exists(test_image):
            os.makedirs(test_image)
        test_label = os.path.join(self.label_path, 'test', 'labels')
    ...
    def push_into_file(self, file, images, labels, suffix='.jpg'):
        """
        ...
        for filename in file:
            image_file = os.path.join(self.label_path, filename + suffix)
            label_file = os.path.join(self.label_path, filename + '.txt')
    ...
    def json2txt(self, classes, txt_Name='allfiles', suffix='.jpg'):
        """
        ...
        :return:
        """
        store_json = os.path.join(self.label_path, 'json')
        if not os.path.exists(store_json):
            os.makedirs(store_json)
    
        _, _, _, files = self.split_dataset(self.label_path)
        if not os.path.exists(os.path.join(self.label_path, 'tmp')):
            os.makedirs(os.path.join(self.label_path, 'tmp'))
        list_file_path = os.path.join(self.label_path,'tmp/%s.txt' % txt_Name)
        list_file = open(list_file_path, mode='w')
        for json_file_ in files:
            json_filename = os.path.join(self.label_path, json_file_ + ".json")
            imagePath = os.path.join(self.label_path, json_file_ + suffix)
            list_file.write('%s\n' % imagePath)
            out_file = open('%s/%s.txt' % (self.label_path, json_file_), 'w')
    ...
    def create_yaml(self, classes, label_path, isUseTest=False):
        nc = len(classes)
        if not isUseTest:
            desired_caps = {
                'path': label_path,
    ...
    def ChangeToYolo5(self, suffix='.jpg', test_size=0.04, isUseTest=False):
        """
        生成最终标准格式的文件
        :param test_size: 分割测试集或验证集的比例
        :param label_path:当前文件路径
        ...
        train_files, val_files, test_file, files = self.split_dataset(test_size=test_size, isUseTest=isUseTest)
        classes = self.get_all_class(files)
        
        self.json2txt(classes)
        self.create_yaml(classes, isUseTest=isUseTest)
        train_image, train_label, val_image, val_label, test_image, test_label = self.create_save_file(label_path)
        self.push_into_file(train_files, train_image, train_label, suffix=suffix)
        self.push_into_file(val_files, val_image, val_label, suffix=suffix)
        if test_file is not None:
            self.push_into_file(test_file, test_image, test_label, suffix=suffix)
